/**
 * Sklentr — Pricing hero: typewriter reveal for the H1 on load.
 * Types the headline character-by-character, preserving the gold accent
 ...
 * (Mirrors svc-hero.js, scoped to the pricing hero.)
 */
(function () {
	'use strict';

	var title = document.querySelector('.pr-hero__title');
	if (!title) {
		return;
	}
	...
	Array.prototype.forEach.call(title.childNodes, function (node) {
		var accent = node.nodeType === 1 && node.classList && node.classList.contains('pr-hero__accent');
	...
	caret.className = 'pr-hero__caret';
	...
	function step() {
		...
		if (!curSpan || tk.accent !== curAccent) {
			...
			if (tk.accent) {
				curSpan.className = 'pr-hero__accent';
			}
